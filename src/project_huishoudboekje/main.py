...
from project_huishoudboekje.read_rabo import ReadRabo, ReadRaboPdf
...
def run():
    ...
    filenames_rabobank_pdf = next(os.walk(GeneralSettings.project_path / f'data/Rabobank (pdf)'), (None, None, []))[2]
    ...
    if filenames_rabobank_pdf:
        df = ReadRaboPdf().run(filenames_rabobank_pdf)
        df = FindCategory().run(df, 'Rabobank')
        StoreResults().run(df)
